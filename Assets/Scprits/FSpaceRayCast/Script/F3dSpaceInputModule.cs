...
namespace UnityEngine.EventSystems
{
    ...
    public class F3dSpaceInputModule : PointerInputModule
    {
        public bool is3D;
        [NonSerialized]
        ...
        //[NonSerialized]
        public F3dSpaceRaycaster activeGraphicRaycaster;
        ...
        {
            base.Start();
            FSpace.FCore.EventKey1Up += Key0Up;
            FSpace.FCore.EventKey1Down += Key0Down;
        }
        ...
        virtual protected MouseState GetGazePointerData()
        {
            ...
            F3dSpacePointerEventData leftData;
            GetPointerData(kMouseLeftId, out leftData, true);
            leftData.Reset();
            //Now set the world space ray. This ray is what the user uses to point at UI elements
            leftData.worldSpaceRay = new Ray(FSpace.FCore.penPosition, FSpace.FCore.penDirection);
            ...
            F3dSpaceRaycaster ovrRaycaster = raycast.module as F3dSpaceRaycaster;
        ...
        protected Dictionary<int, F3dSpacePointerEventData> m_VRRayPointerData = new Dictionary<int, F3dSpacePointerEventData>();


        protected bool GetPointerData(int id, out F3dSpacePointerEventData data, bool create)
        {
            if (!m_VRRayPointerData.TryGetValue(id, out data) && create)
            {
                data = new F3dSpacePointerEventData(eventSystem)
                {
                    pointerId = id,
                };
        ...
        protected MouseState GetMousePointerData(int id)
        {
            ...
            if (created)
            {
                leftData.position = is3D ? new Vector2(Input.mousePosition.x / 2, Input.mousePosition.y) : new Vector2(Input.mousePosition.x, Input.mousePosition.y);
            }
